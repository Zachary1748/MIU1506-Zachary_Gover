...
NSString * k1506_AboutMe_Gover_Zachary$ModuleRequireFormat = @"(function(exports){"
...
@implementation k1506_AboutMe_Gover_ZacharyObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k1506_aboutme_gover_zachary);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_k1506_aboutme_gover_zachary gc];
}
...
{
	// create k1506_AboutMe_Gover_Zachary global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k1506_AboutMe_Gover_Zachary" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k1506_aboutme_gover_zachary = [[k1506_AboutMe_Gover_ZacharyObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k1506_aboutme_gover_zachary];
	
	NSString *_k1506_aboutme_gover_zacharyNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k1506_aboutme_gover_zacharyNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k1506_aboutme_gover_zachary valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k1506_AboutMe_Gover_Zachary$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k1506_aboutme_gover_zachary.kroll" 
