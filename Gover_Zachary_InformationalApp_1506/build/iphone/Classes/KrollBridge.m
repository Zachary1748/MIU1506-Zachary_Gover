...
NSString * Gover_Zachary_InformationalApp_1506$ModuleRequireFormat = @"(function(exports){"
...
@implementation Gover_Zachary_InformationalApp_1506Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_gover_zachary_informationalapp_1506);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_gover_zachary_informationalapp_1506 gc];
}
...
{
	// create Gover_Zachary_InformationalApp_1506 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Gover_Zachary_InformationalApp_1506" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_gover_zachary_informationalapp_1506 = [[Gover_Zachary_InformationalApp_1506Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_gover_zachary_informationalapp_1506];
	
	NSString *_gover_zachary_informationalapp_1506NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _gover_zachary_informationalapp_1506NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_gover_zachary_informationalapp_1506 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Gover_Zachary_InformationalApp_1506$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.gover_zachary_informationalapp_1506.kroll" 
