...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kGover_Zachary_InformationalApp_1506UserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kGover_Zachary_InformationalApp_1506UserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a Gover_Zachary_InformationalApp_1506 app using the node.js CLI
